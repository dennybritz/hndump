...
// Wait this many milliseconds before we fetch an updated item.
// If we try fetching immediately after item creation we get back null.
// Probably eventual consistency on the Firebase side.
var ITEM_FETCH_DELAY = 2500;
...
ItemWatcher.prototype.fetchItem = function(itemId, callback) {
  Promise.delay(ITEM_FETCH_DELAY).then(function(){
...
ItemWatcher.prototype.updateMaxItem = function(maxItem) {
  ...
  itemsToFetch.forEach(function(itemId){
    this.fetchItemAsync(itemId).then(_.partial(this.emitItemCreatedEvent, itemId));
  }.bind(this));
};
